...
setwd("/Users/okonechn/work/spatialMB/")

# script designed for automated giotto analysis

resDir = "/Users/okonechn/work/spatialMB/MB_spatial_results/"

# requires Seurat object, spatial coords saved separately + cluster annotation
...
# WARNING: in case no pca/umap, e.g. for sub-blocks
dim_reduc = NULL
...
temp_dir = paste0("giottoAutoRes/giotto_",resId,"_",sId,"/")
...
# compute PCA, UMAP in case it's subblock

#gMb <- calculateHVG(gobject = gMb) # does not make sense

gMb <- runPCA(gobject = gMb,ncp=20, genes_to_use = fDataDT(gMb)$gene_ID)
#screePlot(gMb, ncp = 20)
plotPCA(gMb)

gMb <- runUMAP(gMb, dimensions_to_use = 1:10,n_threads = 2)
plotUMAP(gobject =gMb)


# annotate cell types: assumes assigned cluster in the object meta.data
gMb <- addCellMetadata(gobject = gMb,new_metadata = sobject@meta.data$assigned_cluster, vector_name = "cluster") 

head(pDataDT(gMb))

gMb <- createNearestNetwork(gobject = gMb, dimensions_to_use = 1:5, k = 5)
plotUMAP( gMb ,cell_color = "cluster", show_NN_network = T) # did not work without fix
...
spatGenePlot(gMb, expression_values = 'scaled', 
             ...
             show_network = F, network_color = 'lightgrey', point_size = 2,
             cow_n_col = 1)
...
saveRDS(gMb,file=paste0(resDir, resId,"_",sId,".giotto_res.rds"))
