...
#from scUtilities import scfunctions_v2 as scfuncs
#from scUtilities import scUtilityClasses_v2 as scu
...
#from pathos.multiprocessing import from pathos.pp import ParallelPool as Pool
...
def plot_umap(data):
    ...
    def _plot_umap(act):
        ...
        dirpath = act.paths['actdir'] / 'umaps'
...
def plot_meta_barplots(data, col1, col2):
    ...
    if col2 != None: 
        ...
        ax = meta.groupby(col1)[col1].count().plot.bar(title="Frequencies", figsize=(18,6), color = 'orange')
        _ = ax.set_xlabel(col1)
        _ = ax.set_ylabel('Frequency')
    print(ax)
...
def get_subsets(ds, dataset_class) -> tuple:
    ...
    for metaCol in ds.analysis_params['subs']: 
        ...
        def subset(cond): # cond = (1,2)
            ...
            # as its own dataset:
            path = ds._paths['analysispath_local'] / subsName / 'data' # MBEN/v01/human/sn/all_t/subsets/clust/
            if not exists(path):
                makedirs(path)
            path = str(path)
